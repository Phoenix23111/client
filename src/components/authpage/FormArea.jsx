  ...
  return (
    <div className="w-full flex  justify-center items-center lg:pt-6   lg:px-56">
      <div className="relative flex  transition-all duration-700 overflow-hidden  rounded-2xl shadow-black shadow-2xl backdrop-brightness-110 bg-secondary/40 backdrop-blur-md w-full h-[75vh]  ">
        <div
          className={` max-sm:hidden absolute top-0 left-0 h-full w-[55%] flex flex-1 col-span-5 rounded-l-2xl transition-all duration-700 ${
            toggleState === "login"
        ...
        <div
          className={`absolute top-0 right-0 h-full md:w-[45%]  transition-all duration-700 flex flex-1 ${
            toggleState === "login"
              ? "translate-x-0 opacity-100 z-50"
              : "-translate-x-full opacity-0 z-0"
          } ${toggleState === "signup" ? "max-sm:hidden" : ""} `}
        >
          <LoginForm handletogglestate={handleToggleState} />
        </div>

        <div
          className={`absolute top-0 left-0 h-full md:w-[45%]   transition-all duration-700 flex flex-1 ${
            toggleState === "login"
              ? "translate-x-full opacity-0 z-0"
              : "-translate-x-0 opacity-100 z-50"
          } ${toggleState === "login" ? "max-sm:hidden" : ""}  `}
        >
          <SignUpForm handletogglestate={handleToggleState} />
        </div>
        <div
          className={` max-sm:hidden absolute top-0 right-0 h-full w-[55%] flex flex-1 col-span-5 rounded-l-2xl transition-all duration-700 ${
            toggleState === "login"
              ? "-translate-x-full opacity-0 z-0"
              : "-translate-x-0 opacity-100 z-50"
          }  `}
